...
def dice_score(input, target): # input: 16x2x352x352 target 16x2x352x352
    batch_size = target.size(0)   # trainn theo batch 
    dice_score = 0 
    smooth = 1.
    
    neo_tflat = target[:, [0], ...]
    non_neo_tflat = target[:, [1], ...]
    seg_tflat = (neo_tflat + non_neo_tflat).view(batch_size, -1)  # view -> flatten, kiểu reshape , inline
    for pre in input:
        ...
        intersection_neo = (seg_iflat * seg_tflat).sum(axis=1)  # tổng theo chiều dọc 3*4 -> (1*4)
